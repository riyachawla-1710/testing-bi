...
with not_invoiced as (
    ...
    left join {{ source('bi_analytics', 'labatt_orderrevenue') }} lb on lb.orderguid = od.id
    left join {{ source('bi_analytics', 'ordercharges_bs_vw') }} ob on ob.orderguid = od.id
    ...
    left join {{ source('bi_analytics', 'keurig_shuntingrevenue_vw') }} ks
